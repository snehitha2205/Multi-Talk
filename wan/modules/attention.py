...
def flash_attention(
    ...
    if not (FLASH_ATTN_3_AVAILABLE or FLASH_ATTN_2_AVAILABLE):
        raise RuntimeError("Flash attention library not available on this system (CPU-only).")
...
def attention(q, k, v, q_lens=None, k_lens=None, dropout_p=0., softmax_scale=None,
    ...
    if FLASH_ATTN_2_AVAILABLE or FLASH_ATTN_3_AVAILABLE:
        return flash_attention(q, k, v, q_lens, k_lens, dropout_p, softmax_scale,
                               q_scale, causal, window_size, deterministic, dtype, fa_version)
    else:
        if q_lens or k_lens:
            warnings.warn("Padding mask is disabled using scaled_dot_product_attention.")
...
class SingleStreamAttention(nn.Module):
    def __init__(self, dim, encoder_hidden_states_dim, num_heads,
                 qkv_bias, qk_norm, norm_layer, attn_drop=0.0, proj_drop=0.0, eps=1e-6):
        super().__init__()
        ...
        self.q_norm = norm_layer(self.head_dim, eps=eps) if qk_norm else nn.Identity()
        self.add_k_norm = norm_layer(self.head_dim, eps=eps) if qk_norm else nn.Identity()
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(dim, dim)
        self.proj_drop = nn.Dropout(proj_drop)

    def forward(self, x, encoder_hidden_states, shape=None, enable_sp=False, kv_seq=None):
        # Implementation simplified: q/k/v prep + attention call
        B, N, C = x.shape
        q = self.q_linear(x)
        q = q.view(B, N, self.num_heads, self.head_dim).permute(0,2,1,3)
        if self.qk_norm: q = self.q_norm(q)

        _, N_a, _ = encoder_hidden_states.shape
        kv = self.kv_linear(encoder_hidden_states)
        kv = kv.view(B, N_a, 2, self.num_heads, self.head_dim).permute(2,0,3,1,4)
        k, v = kv.unbind(0)
        if self.qk_norm: k = self.add_k_norm(k)

        # Memory-efficient attention
        q_r = rearrange(q, "B H M K -> B M H K")
        k_r = rearrange(k, "B H M K -> B M H K")
        v_r = rearrange(v, "B H M K -> B M H K")
        if xformers is not None:
            out = xformers.ops.memory_efficient_attention(q_r, k_r, v_r)
        else:
            out = torch.nn.functional.scaled_dot_product_attention(q_r.permute(0,2,1,3),
                                                                    k_r.permute(0,2,1,3),
                                                                    v_r.permute(0,2,1,3))
            out = out.permute(0,2,1,3)
        x = rearrange(out, "B M H K -> B H M K")
...
class SingleStreamMutiAttention(SingleStreamAttention):
    def __init__(self, dim, encoder_hidden_states_dim, num_heads,
        ...
        super().__init__(dim, encoder_hidden_states_dim, num_heads,
                         qkv_bias, qk_norm, norm_layer, attn_drop, proj_drop, eps)
        self.class_interval = class_interval
        self.class_range = class_range
        self.rope_h1 = (0, self.class_interval)
        self.rope_h2 = (self.class_range - self.class_interval, self.class_range)
        self.rope_bak = self.class_range // 2
        self.rope_1d = RotaryPositionalEmbedding1D(self.head_dim)

    def forward(self, x, encoder_hidden_states, shape=None, x_ref_attn_map=None, human_num=None):
        encoder_hidden_states = encoder_hidden_states.squeeze(0)
        if human_num == 1:
            return super().forward(x, encoder_hidden_states, shape)
        ...
        # Normalize attention map and apply rotary embeddings
        max_values = x_ref_attn_map.max(1).values[:, None, None]
        min_values = x_ref_attn_map.min(1).values[:, None, None]
        max_min_values = torch.cat([max_values, min_values], dim=2)
        human1_max_value, human1_min_value = max_min_values[0,:,0].max(), max_min_values[0,:,1].min()
        ...
        human1 = normalize_and_scale(x_ref_attn_map[0], (human1_min_value, human1_max_value), self.rope_h1)
        human2 = normalize_and_scale(x_ref_attn_map[1], (human2_min_value, human2_max_value), self.rope_h2)
        back = torch.full((x_ref_attn_map.size(1),), self.rope_bak, dtype=human1.dtype).to(human1.device)
        max_indices = x_ref_attn_map.argmax(dim=0)
        normalized_map = torch.stack([human1, human2, back], dim=1)
        ...
        # Process encoder_k
        _, N_a, _ = encoder_hidden_states.shape
        kv = self.kv_linear(encoder_hidden_states).view(B, N_a, 2, self.num_heads, self.head_dim).permute(2,0,3,1,4)
        k, v = kv.unbind(0)
        if self.qk_norm: k = self.add_k_norm(k)
        per_frame = torch.zeros(N_a, dtype=k.dtype, device=k.device)
        per_frame[:N_a//2] = sum(self.rope_h1)/2
        per_frame[N_a//2:] = sum(self.rope_h2)/2
        ...
        k_r = rearrange(k, "B H M K -> B M H K")
        v_r = rearrange(v, "B H M K -> B M H K")
        x = xformers.ops.memory_efficient_attention(q_r, k_r, v_r) if xformers else fallback_memory_efficient_attention(q_r, k_r, v_r)

        x = rearrange(x, "B M H K -> B H M K").transpose(1,2).reshape(B,N,C)
        x = self.proj(x)
        x = self.proj_drop(x)
        x = rearrange(x, "(B N_t) S C -> B (N_t S) C", N_t=N_t)
        return x
